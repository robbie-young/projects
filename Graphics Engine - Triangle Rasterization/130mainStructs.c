/*
    130mainStructs.c
    Example file demonstrating implementation and function of structs in C.
...
struct Employee {
    ...
    double hairColor[3];    /* Yes, an array can be a member of a struct.hi */
};
...
hair, but the design is different. Instead of operating on a copy of an 
Employee, it operates on a pointer to the original. Usually, pointers are 
...
usually 'better style' than ageWithoutPointers. */
...
/* Here's a function that prints a linked list of doubles. I've added the const 
qualifier to the node argument to help protect against accidental 
modifications. The second use of const, on the current variable, is more 
subtle. Try removing the second const and seeing what happens. */
void DoubleNode_print(const DoubleNode *node) {
    const DoubleNode *current = node;
...
/*  By the way, C does not support object-oriented design, but you can fake it 
by combining structs such as DoubleNode with functions such as 
DoubleNode_print. In fact, several object-oriented languages are implemented in 
C, by pursuing this idea to its logical conclusion. Talk to me about that if  
you're interested. */
...
int main(void) {
    ...
    printf("In 2017, the first employee was about %d years old.\n", 
