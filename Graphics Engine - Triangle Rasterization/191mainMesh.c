/*
    191mainMesh.c
    Implements rendering an image using a mesh. Demo involves creation of a two-triangle mesh.
...
/* On macOS, compile with...
    clang 191mainMesh.c 040pixel.o -lglfw -framework OpenGL -framework Cocoa -framework IOKit
On Ubuntu, compile with...
    cc 191mainMesh.c 040pixel.o -lglfw -lGL -lm -ldl
*/
...
void render(meshMesh *mesh) {
    ...
    double d[7] = {200.0, 20.0, 1.0, 0.0, 1.0, 1.0, 1.0};
...
int main(void) {
    ...
    sha.attrDim = 2 + 2 + 3;
    sha.texNum = 1;
    if (meshInitialize(&mesh, 2, 4, sha.attrDim) != 0) {
        return 3;
	}
    render(&mesh);
