/*
	...
	Differs from 260mesh.c by performing a viewport transformation and homogeneous division in meshRender.
...
void meshRender(
	...
	double *attr[3], vary[3][varyDim], varyTransformed[4];
	int *currTriangle;
	...
	for (int i = 0 ; i < mesh->triNum ; i ++) {
		...
		for (int j = 0 ; j < 3 ; j ++) {
			...
			vecScale(4, 1/varyTransformed[3], varyTransformed, vary[j]);
			// printf("[%f, %f, %f]\n", vary[j][0], vary[j][1], vary[j][2]);
		}
