...
void cumulativeSum(int length, const a[]) {
    int i;
    for (i = 1; i < length; i += 1)
        a[i] += a[i - 1];
}

/* In this course, we frequently want to use arrays in a read-only manner. To 
avoid accidentally altering the contents of an array that is meant to be read-
only, we prepend the const qualifier to the array declaration. To practice this 
concept, edit the cumulativeSum function above, replacing 'double a[]' with 
'const double a[]'. Then recompile. Did the compiler give you an error or 
warning? (Not all compilers do.) */
...
int main(void) {
    ...
    printf("Here are the elements of myOtherArray:\n");
    for (i = 0; i < 4; i += 1)
        printf("    myOtherArray[%d] = %f.\n", i, myOtherArray[i]);
