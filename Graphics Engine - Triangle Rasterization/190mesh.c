...
void meshRender(
        ...
        const texTexture *tex[]) {
            ...
            for (int i = 0 ; i < mesh->triNum ; i ++) {
                int currTri[3] = {mesh->tri[i*3], mesh->tri[i*3+1], mesh->tri[i*3+2]};
                double attr1[attrDim]; double attr2[attrDim]; double attr3[attrDim];
				/* sets the attributes of each vertex */
                for (int j = 0 ; j < attrDim; j++) {
                    attr1[j] = mesh->vert[attrDim * currTri[0] + j];
                }
                for (int j = 0 ; j < attrDim; j++) {
                    attr2[j] = mesh->vert[attrDim * currTri[1] + j];
                }
                for (int j = 0 ; j < attrDim; j++) {
                    attr3[j] = mesh->vert[attrDim * currTri[2] + j];
                }
                triRender(sha, unif, tex, attr1, attr2, attr3);
            }
}