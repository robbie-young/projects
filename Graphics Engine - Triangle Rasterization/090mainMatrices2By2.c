...
/* On macOS, compile with...
    clang 090mainMatrices2By2.c
On Ubuntu, compile with...
    cc 090mainMatrices2By2.c
*/

#include <stdio.h>

/* You can think of a matrix as a rectangular grid of numbers, or as an array of 
arrays. Here's a function that takes a 2x2 matrix as input, and prints it. The 
syntax m[i][j] refers to the (i, j)th entry of the matrix m --- that is, the 
entry in row i and column j, both indexed from 0. */
...
/* Remember from 070mainArrays.c that you don't have to declare the length of 
the array in the interface to a function. For example, you can have a parameter 
...
void vec2Print(const double v[2]) {
    printf("%f\n%f\n", v[0], v[1]);
}
