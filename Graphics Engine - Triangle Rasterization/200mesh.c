/*
	200mesh.c
	Creates the meshMesh struct and defines methods to implement it, including meshRender. Differs from 190mesh.c be implementing varyings.
...
void meshRender(
        ...
        const texTexture *tex[]) {
            ...
            for (int i = 0 ; i < mesh->triNum ; i ++) {
				...
				shadeVertex(sha->unifDim, unif, attrDim, attr[0], varyDim, vary1);
				shadeVertex(sha->unifDim, unif, attrDim, attr[1], varyDim, vary2);
				shadeVertex(sha->unifDim, unif, attrDim, attr[2], varyDim, vary3);
                triRender(sha, unif, tex, vary1, vary2, vary3);
            }
}